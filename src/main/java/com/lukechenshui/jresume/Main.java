...
//        createPDF(runtime.getOutputHtmlFile().getAbsolutePath());
    ...
    public static void createPDF(String path, Runtime runtime){
    ...
    public static void createInlineHTML(Runtime runtime){
        try {
            ...
            process.waitFor();
            createPDF(runtime.getOutputHtmlFile("resume_inline.html").getAbsolutePath(), runtime);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private static void startListeningAsServer() throws Exception {
        ...
        post("/webresume/html", "application/json", (request, response) -> {
            return generateWebResumeInRoute("default", request, response, WEBREQUEST_TYPE.HTML);
        });

        post("/webresume/pdf", "application/json", (request, response) -> {
            return generateWebResumeInRoute("default", request, response, WEBREQUEST_TYPE.PDF);
        });
...
//        });
    ...
    private static Object generateWebResumeInRoute(String theme, Request request, Response response, WEBREQUEST_TYPE type) throws Exception {
        int currentReqId = outputPrefixNumber.incrementAndGet();
        File outputDirectory = new File("data/jresume" + currentReqId + ".tmp");
        Runtime runtime = new Runtime(outputDirectory, currentReqId, config, type);
